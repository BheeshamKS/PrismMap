...
function LogPanel({ logs, loading }) {
  ...
  return (
    <div className="border border-zinc-800 bg-zinc-950 px-4 py-3 space-y-1.5">
      {logs.map((msg, i) => (
        <div key={i} className="flex items-center gap-2.5 font-mono text-sm animate-fade-in">
...
export default function App() {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:8001/analyze', {
        ...
        body: JSON.stringify({ repo_url: repoUrl, bug_description: bugDescription }),
      })
  ...
  const canSubmit = repoUrl.trim() && bugDescription.trim() && status !== 'loading'
    ...
    <div className="min-h-screen bg-black text-white">
      <div className="max-w-2xl mx-auto px-4 py-16 space-y-8">
        ...
        <div className="space-y-1">
          <h1 className="font-mono text-xl font-semibold tracking-tight">prism map</h1>
          <p className="font-mono text-sm text-zinc-600">context-aware prompt builder</p>
        </div>
